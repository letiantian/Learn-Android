...
public class ColorArt {
    private final double COLOR_THRESHOLD_MINIMUM_PERCENTAGE = 0.01;  // 颜色分布在0、1之间
    private final double EDGE_COLOR_DISCARD_THRESHOLD = 0.3;   // 某比值的阈值
    private final float MINIMUM_SATURATION_THRESHOLD = 0.15f;  // 某比值的阈值
    ...
    private int findEdgeColor() {
        ...
        mImageColors = new HashBag<Integer>();  // 图像上所有像素点的颜色
        HashBag<Integer> leftImageColors = new HashBag<Integer>();   // 图像上所有最左侧像素点的颜色
        ...
        ArrayList<CountedColor> sortedColors = new ArrayList<CountedColor>();  // 最终会根据颜色出现次数进行排序

        // 最左侧颜色出现次数的阈值，只要出现次数大于阈值的颜色
        ...
        while (iterator.hasNext()) {
            ...
            if (colorCount < randomColorThreshold) {  // 小于阈值，不要了
                continue;
            }
        ...
        Collections.sort(sortedColors);  // 排序

        Iterator<CountedColor> sortedColorIterator = sortedColors.iterator();
        if (!sortedColorIterator.hasNext()) {  // 防止集合中没有元素，~~
            return Color.BLACK;
        }

        CountedColor proposedEdgeColor = sortedColorIterator.next(); // 得到第一个颜色，即出现次数最多的颜色
        // 若proposedEdgeColor偏白色和黑色，则尽量不要
        if (!proposedEdgeColor.isBlackOrWhite()) {
            return proposedEdgeColor.getColor();
        }

        // 如果proposedEdgeColor不合适
    ...
    private void findTextColors(HashBag<Integer> colors) {
        ...
        boolean findDarkTextColor = !isDarkColor(mBackgroundColor); // 要不要去找较深的文字颜色；背景色深的话，就不应该去找深文本色

        while (iterator.hasNext()) {
            currentColor = iterator.next();
            currentColor = colorWithMinimumSaturation(currentColor, MINIMUM_SATURATION_THRESHOLD);
            if (isDarkColor(currentColor) == findDarkTextColor) {  // 找到了合适的颜色
    ...
    /**
     * 将rgb颜色转换成gray，根据gray的大小判断RGB颜色是否是深色
     * @param color
     * @return
     */
    ...
    /**
     * 是否是对比鲜明的颜色。将前景色和背景色都转换成RGB颜色，然后求两者的比值，若超过阈值，则认为颜色形成鲜明对比。
